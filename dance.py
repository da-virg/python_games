...
def update_dancer(move):
    ...
    if not game_over:
        if move == 0:
            ...
            clock.schedule(reset_dancer, 1.5)
        elif move == 2:
            up.image = "down-lit"
...
def display_moves():
    ...
    global say_dance, show_countdown, current_level
    if display_list:
...
def generate_moves():
    ...
    global show_countdown, say_dance, display_list
    count = 30
    move_list = []
    say_dance = False
    ...
    print("Generate_moves: display_list =", display_list)
    show_countdown = True
    countdown()
    return
